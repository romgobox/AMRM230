...
class m230():
    ...
    def whU(self, whAdr):
        """
        Метод предназначен для чтения мгновенных значений напряжения (В), возвращает float в случае чтения
        по одной фазе (phase = 1(2,3) соответсвует фазам A,B,C счетчика) или словарь с ключами 1,2,3
        """
    ...
    def whI(self, whAdr):
        """
        Метод предназначен для чтения мгновенных значений тока (А), возвращает float в случае чтения
        по одной фазе (phase = 1(2,3) соответсвует фазам A,B,C счетчика) или словарь с ключами 1,2,3
    ...
    def whP(self, whAdr, en='P'):
        """
        Метод предназначен для чтения мгновенных значений мощност.
        ...
        ph = {
            ...
            'Q':{0:'\x00', 1:'\x21', 2:'\x22', 3:'\x23',},
            'S':{0:'\x00', 1:'\x21', 2:'\x22', 3:'\x23',},
        }
        P = {}
        for p in ph[en]:
